                             ...
                             AVFilterContext **sink)
{
    ...
    printf("[init_filter_graph()] - Initializing filters...\n");
    ...
    err = avfilter_graph_create_filter(&abuffer0_ctx, abuffer0, "src0", args, NULL, filter_graph);
    if (err < 0) {
    ...
    *sink  = abuffersink_ctx;
    ...
    printf("[init_filter_graph()] - Ending method\n");
                              ...
                              int *data_present, int *finished)
{
    ...
    while (error >= 0) {
        ...
        if (error == AVERROR(EAGAIN) || error == AVERROR_EOF) {
            fprintf(stderr, "Fatal Error -> AVERROR(EAGAIN) | AVERROR_EOF - %s\n", get_error_text(error)); 
            return error;
        } else if (error < 0) {
        ...
        // memcpy(data_present, frame->data, sizeof(frame->data));
...
static int process_all(){
    printf("[process_all()] - Starting method...\n");
    int ret = 0;
    ...
    // These buffers store the data from the audio files 
    AVFilterContext* buffer_contexts[2];
    buffer_contexts[0] = src0;
    buffer_contexts[1] = src1;
   
    // flags to check if an audio file was already processed 
    ...
    printf("[process_all()] - nb_inputs -> %d\n", nb_inputs);
    printf("[process_all()] - nb_finished -> %d\n", nb_finished);
    
    while (nb_finished < nb_inputs) { // Processing every audio file, one at a time 
        int data_present_in_graph = 0;
        
        for (int i=0; i<nb_inputs; i++) {
            printf("[process_all()] - Tracing for -> %d\n", i);
            ...
            if (finished && !data_present) { // Processing delayed data at the end of the file
                input_finished[i] = 1;
                nb_finished++;
                printf("[process_all()] - Tracing nb_finished -> %d\n", nb_finished);

                ret = 0;
                av_log(NULL, AV_LOG_INFO, "Input n°%d finished. Write NULL frame \n", i);
                
                ret = av_buffersrc_write_frame(buffer_contexts[i], NULL); // End of the file, so frame is NULL
                if (ret < 0) {
                ...
                ret = av_buffersrc_write_frame(buffer_contexts[i], frame); // Storing decoded frame
                if (ret < 0) {
        ...
        if (data_present_in_graph){
            ...
            while (1) {
                ...
                // Encoding the final audio frame
    ...
    printf("[process_all()] - Ending method...\n");
