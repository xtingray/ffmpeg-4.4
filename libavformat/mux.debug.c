...
{
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9\n");

    int delay = FFMAX(st->codecpar->video_delay, st->internal->avctx->max_b_frames > 0);
    int i;
    int frame_size;

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9A\n");
    ...
    if (!s->internal->missing_ts_warning &&
        ...
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9B\n");
        av_log(s, AV_LOG_WARNING,
    ...
    if (s->debug & FF_FDEBUG_TS) {
        av_log(s, AV_LOG_DEBUG, "compute_muxer_pkt_fields: pts:%s\n",
            av_ts2str(pkt->pts));
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 10A\n");

        av_log(s, AV_LOG_DEBUG, "compute_muxer_pkt_fields: dts:%s\n",
            av_ts2str(pkt->dts));
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 10B\n");

        av_log(s, AV_LOG_DEBUG, "compute_muxer_pkt_fields: cur_dts:%s\n",
            av_ts2str(st->cur_dts));
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 10C\n");

        av_log(s, AV_LOG_DEBUG, "compute_muxer_pkt_fields: b:%d\n",
            delay);
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 10D\n");

        av_log(s, AV_LOG_DEBUG, "compute_muxer_pkt_fields: size:%d\n",
            pkt->size);
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 10E\n");

        av_log(s, AV_LOG_DEBUG, "compute_muxer_pkt_fields: st:%d\n",
            pkt->stream_index);
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 10F\n");
    }

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9C\n");

    if (pkt->pts == AV_NOPTS_VALUE && pkt->dts != AV_NOPTS_VALUE && delay == 0)
        pkt->pts = pkt->dts;

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9D\n");
...
//        pkt->pts= st->cur_dts;
    ...
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9E\n");
    ...
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9F\n");
    ...
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9G\n"); 

    if (s->debug & FF_FDEBUG_TS)
        av_log(s, AV_LOG_DEBUG, "av_write_frame: pts2:%s dts2:%s\n",
            av_ts2str(pkt->pts), av_ts2str(pkt->dts));

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9GA\n");
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9GA -> pkt->dts -> %d\n", pkt->dts);

    st->cur_dts = pkt->dts;

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9GB\n"); 

    if (st) {
        av_log(s, AV_LOG_INFO, "write_packets_common() - st is NOT NULL\n");
        if (st->internal) { 
            av_log(s, AV_LOG_INFO, "write_packets_common() - st->internal is NOT NULL!!!\n"); 
            if (st->internal->priv_pts) {
                av_log(s, AV_LOG_INFO, "write_packets_common() - st->internal->priv_pts is NOT NULL!!!\n"); 
                st->internal->priv_pts->val = pkt->dts;
            } else {
                av_log(s, AV_LOG_INFO, "write_packets_common() - st->internal->priv_pts is NULL!!!\n");
            }
        } else {
            av_log(s, AV_LOG_INFO, "write_packets_common() - Warning: st->internal is NULL!!!\n"); 
        }
    } else {
        av_log(s, AV_LOG_INFO, "write_packets_common() - Warning: st is NULL\n");
    }

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 9H\n");
...
static int write_packet_common(AVFormatContext *s, AVStream *st, AVPacket *pkt, int interleaved)
{
    ...
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 8\n");

    if (s->debug & FF_FDEBUG_TS)
        av_log(s, AV_LOG_DEBUG, "%s size:%d dts:%s pts:%s\n", __FUNCTION__,
               pkt->size, av_ts2str(pkt->dts), av_ts2str(pkt->pts));

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 8A\n");

    guess_pkt_duration(s, st, pkt);

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 8B\n");
    ...
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 8C\n");

    if (interleaved) {
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 8D\n");
        if (pkt->dts == AV_NOPTS_VALUE && !(s->oformat->flags & AVFMT_NOTIMESTAMPS))
            return AVERROR(EINVAL);
        return interleaved_write_packet(s, pkt, 0);
    } else {
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 8E\n");
...
{
    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 3\n");
    ...
    if (st) {
        av_log(s, AV_LOG_INFO, "TRACE 3A1 -> ST is NOT NULL\n"); 
    } else {
        av_log(s, AV_LOG_INFO, "TRACE 3A2 -> Warning! ST is NULL\n"); 
    }

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 4\n");

    ret = prepare_input_packet(s, st, pkt);
    if (ret < 0)
        return ret;

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 5\n");

    ret = check_bitstream(s, st, pkt);
    if (ret < 0)
        return ret;

    av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 6\n");

    if (st->internal->bsfc) {
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 7\n");
        return write_packets_from_bsfs(s, st, pkt, interleaved);
    } else {
        av_log(s, AV_LOG_INFO, "write_packets_common() - TRACE 7A\n");
...
int av_interleaved_write_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    av_log(s, AV_LOG_INFO, "av_interleaved_write_frame() - TRACE 1 \n"); 

    if (pkt) {
        av_log(s, AV_LOG_INFO, "av_interleaved_write_frame() - TRACE 2A \n");
        ret = write_packets_common(s, pkt, 1/*interleaved*/);
        if (ret < 0)
            av_packet_unref(pkt);
        return ret;
    } else {
        av_log(s, AV_LOG_INFO, "av_interleaved_write_frame() - TRACE 2B \n");
